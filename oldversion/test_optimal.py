...
def score_text(text):
    ...
    if len(words_lower) > 0:
        ...
        # 調整: 人類文本也可能有高 TTR，所以降低權重
        vocab_score = max(0, min((vocab_ratio - 0.55) / 0.25, 1)) * 0.24
    ...
    if len(sentences) > 1:
        ...
        # 調整: 人類也可以有規律的句子結構
        consistency_score = max(0, 1 - min(cv, 1.5) / 1.5) * 0.24
    ...
    if literary_count > 0:
        literary_penalty = min(literary_count * 0.24, 0.50)
...
print(f"\n{BLUE}{'='*70}")
print("  AI 偵測系統 - 最優版 (權重: 24-24-8-8-24-12)")
print(f"{'='*70}{END}\n")
...
if success == len(test_cases):
    print(f"{GREEN}✅ 所有測試通過！系統準確度達 100%！{END}\n")
else:
    print(f"{YELLOW}⚠️  部分測試未通過，需進一步調整{END}\n")
