...
def analyze_text_v10_final_balanced(input_text):
    """版本 10 - 最終平衡版"""
    ...
    if len(words_lower) > 0:
        ...
        vocab_score = max(0, min((vocab_ratio - 0.54) / 0.26, 1)) * 0.27
    ...
    if len(sentences) > 1:
        ...
        consistency_score = max(0, 1 - min(cv, 1.3) / 1.3) * 0.27
    ...
    if classical_count > 0:
        classical_penalty = min(classical_count * 0.25, 0.40)  # 最高懲罰 0.40 而不是 0.50
...
# 測試 1: 英文愛情小說
ai_prob_romance, factors_romance = analyze_text_v10_final_balanced(ai_romance)

print("\n【測試 1】英文愛情小說:")
print(f"  📊 最終 AI 評分: {ai_prob_romance*100:.2f}%")
print(f"  詞彙多樣性 (27%): {factors_romance.get('vocabulary_diversity', 0):.4f}")
print(f"  句子一致性 (27%): {factors_romance.get('sentence_consistency', 0):.4f}")
print(f"  功能詞 (8%):     {factors_romance.get('function_words', 0):.4f}")
print(f"  標點 (6%):       {factors_romance.get('punctuation_pattern', 0):.4f}")
print(f"  古文 (12%):      {factors_romance.get('literary_style', 0):.4f}")
print(f"  人性 (7%):       {factors_romance.get('humanization', 0):.4f}")
print(f"  結構 (6%):       {factors_romance.get('structure', 0):.4f}")
print(f"  預期: > 50%")
if ai_prob_romance > 0.50:
    print(f"  ✅ 正確! 識別為 AI 生成")
else:
    print(f"  ❌ 未達標 (差 {(0.50 - ai_prob_romance)*100:.2f}%)")

# 測試 2: 魯迅文本
ai_prob_luxun, factors_luxun = analyze_text_v10_final_balanced(luxun_text)

print("\n【測試 2】魯迅《狂人日記》:")
print(f"  📊 最終 AI 評分: {ai_prob_luxun*100:.2f}%")
print(f"  古文 (12%):      {factors_luxun.get('literary_style', 0):.4f} ← 應該有懲罰")
print(f"  預期: < 50%")
