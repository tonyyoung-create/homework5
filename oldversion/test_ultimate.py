...
def score_text(text):
    ...
    text_len = len(words_lower)
    
    # 0. 短文本偏好 (人類傾向寫短文本) - 4%
    # 短於 100 詞 = 更可能是人類
    if text_len < 100:
        short_text_reduction = min(text_len / 100, 1.0) * 0.04
        ai_score -= short_text_reduction
        score_factors['short_text_adjustment'] = -short_text_reduction
    else:
        score_factors['short_text_adjustment'] = 0.0
    ...
    if len(sentences) > 1:
        ...
        consistency_score = max(0, 1 - min(cv, 1.5) / 1.5) * 0.22
    ...
    if literary_count > 0:
        literary_penalty = min(literary_count * 0.28, 0.50)
