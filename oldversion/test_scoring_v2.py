...
def score_text(text):
    ...
    if len(words_lower) > 0:
        ...
        vocab_score = max(0, min((vocab_ratio - 0.5) / 0.3, 1)) * 0.20
    ...
    if len(sentences) > 1:
        ...
        consistency_score = max(0, 1 - min(cv, 1.2) / 1.2) * 0.20
    ...
    if any(ord(c) < 128 for c in text):
        ...
        if len(words_lower) > 0:
            ...
            func_score = min(func_ratio / 0.30, 1) * 0.05
            ai_score += func_score
    ...
    if literary_count > 0:
        # 每個文學標記減 0.25 分（提升影響）
        literary_penalty = min(literary_count * 0.25, 0.5)
...
if success == len(test_cases):
    print(f"{GREEN}✅ 所有測試通過！文學風格檢測工作正常！{END}\n")
else:
    print(f"{YELLOW}⚠️  部分測試未通過，需要微調{END}\n")
