...
def score_text(text):
    ...
    if len(words_lower) > 0:
        ...
        vocab_score = max(0, min((vocab_ratio - 0.55) / 0.25, 1)) * 0.22
    ...
    if len(sentences) > 1:
        ...
        consistency_score = max(0, 1 - min(cv, 1.3) / 1.3) * 0.22
    ...
    if any(ord(c) < 128 for c in text):
        ...
        if len(words_lower) > 0:
            ...
            func_score = min(func_ratio / 0.30, 1) * 0.07
            ai_score += func_score
    ...
    if literary_count > 0:
        literary_penalty = min(literary_count * 0.25, 0.50)
    ...
    if question_ratio > 0.15:  # 高於 15% 是人類特徵
        humanization_score += 0.05
    
    # 省略號（...）
    ellipsis_count = text.count('...') + text.count('。。。')
    if ellipsis_count > 0:
        humanization_score += 0.03
    ...
    if personal_count > 1:
        humanization_score += min(personal_count * 0.02, 0.02)
    
    ai_score -= min(humanization_score, 0.10)
    score_factors['humanization'] = -min(humanization_score, 0.10)
...
print(f"\n{BLUE}{'='*70}")
print("  AI 偵測系統 - 頂級版 (加入人性化標記: 22-22-7-6-25-10-8)")
print(f"{'='*70}{END}\n")
...
if success == len(test_cases):
    print(f"{GREEN}✅ 所有測試通過！問題已完全解決！{END}\n")
