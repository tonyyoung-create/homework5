...
def score_text(text):
    ...
    if len(words_lower) > 0:
        ...
        vocab_score = max(0, min((vocab_ratio - 0.50) / 0.30, 1)) * 0.25
    ...
    if len(sentences) > 1:
        ...
        consistency_score = max(0, 1 - min(cv, 1.2) / 1.2) * 0.25
    ...
    if literary_count > 0:
        # 每個標記減 0.26 分
        literary_penalty = min(literary_count * 0.26, 0.52)
...
print(f"\n{BLUE}{'='*70}")
print("  AI 偵測系統 - 改進平衡版 (權重: 25-25-8-7-26-9)")
print(f"{'='*70}{END}\n")
