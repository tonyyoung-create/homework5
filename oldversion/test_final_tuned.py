...
def score_text(text):
    ...
    if len(sentences) > 1:
        ...
        # 調高 CV 基準，使人類文本不易達到高分
        consistency_score = max(0, 1 - min(cv, 1.3) / 1.3) * 0.24
    ...
    if literary_count > 0:
        literary_penalty = min(literary_count * 0.27, 0.54)
    ...
    if len(paragraphs) <= 1:
        ...
        struct_score = max(0, 1 - min(para_cv, 1)) * 0.10
...
print(f"\n{BLUE}{'='*70}")
print("  AI 偵測系統 - 最終調整版 (權重: 24-24-8-7-27-10)")
print(f"{'='*70}{END}\n")
...
if success == len(test_cases):
    print(f"{GREEN}✅ 所有測試通過！問題已解決！{END}\n")
